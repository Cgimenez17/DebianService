...
// Incluir el archivo de generación de contrato
require 'generar_contrato.php';  // Asegúrate de que la ruta sea correcta
        ...
        if (isset($_FILES['cv_empleado']) && $_FILES['cv_empleado']['error'] === UPLOAD_ERR_OK) {
            ...
            if ($fileExtension === 'pdf') {
                ...
                if (move_uploaded_file($fileTmpPath, $destPath)) {
                    ...
                    $queryEmpleados = mysqli_query($mysqli, "INSERT INTO empleados (id_empleado, nombre, apellido, documento, email, telefono, direccion, fecha_ingreso, estado, cvempleado, salario, cargo) 
                        VALUES ('$codigo', '$nombre','$apellido','$ci','$mail', '$tel', '$direccion', now(), 'MANDATORY_CHANGE', '$destPath', $salario , '$cargo')")
                    ...
                    $queryCV = mysqli_query($mysqli, "INSERT INTO curriculum (id_curriculum, id_empleado, archivo_cv, fecha_carga)
                        VALUES('$codigo','$codigo', '$destPath', now())")
                    ...
                    // Definir datos básicos del contrato
                    $tipoContrato = $_POST['tipo_contrato'] ?? 'Indeterminado'; // Cambiar según tus necesidades
                    $fechaInicio = $_POST['fecha_inicio'] ?? date('Y-m-d'); // Fecha de inicio
                    ...
                    echo "Ruta del contrato: $rutaContrato";  // Depuración para verificar la ruta

                    // Guardar el contrato en la base de datos
                    $queryContrato = mysqli_query($mysqli, "
                    INSERT INTO contratos (id_empleado, tipo_contrato, fecha_inicio, fecha_fin, archivo_contrato) 
                    VALUES ('$codigo', '$tipoContrato', '$fechaInicio', " . ($fechafincontrato ? "'$fechafincontrato'" : "NULL") . ", '$rutaContrato')
                    ") or die('Error: ' . mysqli_error($mysqli));

                    // Redirigir con un mensaje de éxito
                    if ($queryEmpleados && $queryUSER && $queryCV && $queryContrato) {
